...
def save_to_excel(cars, filename):
    ...
    for car in cars:
        ...
        ws.cell(row=row, column=9).font = Font(color=color, bold=True)
...
def build_email_html(cars, medians, search_label=""):
    ...
    <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-family:Arial;">
        <tr style="background-color:#2f4f6f;color:white;">
            <th>#</th><th>Title</th><th>Price</th><th>Mileage</th>
            <th>Year</th><th>Location</th><th>Link</th><th>Deal</th>
        </tr>
    """
    for car in cars:
        rating = car.get("Pontszám") or 0
    ...
    for page_num in range(1, 10):
        ...
        for article in articles:
            try:
                ...
                # Link keresés az article HTML-jéből
                link = ""
    ...
    with sync_playwright() as p:
        ...
        for dealer in dealers:
            ...
            for search in dealer["searches"]:
                ...
                cars = scrape_search(page, search["brand"], search["model"],
                                     search["year_from"], search["year_to"], search["country"])
                page.close()
                ...
                for c in cars:
                    yr    = c.get("Év")
                    price = c.get("Ár_num")
                    if not yr or not price:
                        continue
                    yr_only = yr.split("/")[-1]
                    cars_by_year.setdefault(yr_only, []).append(price)

                medians = {}
                for yr, prices in cars_by_year.items():
                    prices = sorted(prices)
                    n = len(prices)
                    medians[yr] = prices[n // 2] if n % 2 == 1 else (prices[n//2-1] + prices[n//2]) / 2
